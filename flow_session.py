...
# --- Konstanta ---
EXPIRED_UPDATE = 10
ICMP_DURATION_THRESHOLD = 5         # detik
ICMP_PACKET_COUNT_THRESHOLD = 5   # jumlah minimal packet sebelum kirim
MACHINE_LEARNING_API = "http://localhost:8000/predict"
GARBAGE_COLLECT_PACKETS = 20
        ...
        if self.url_model:
            ...
            try:
                ...
                if post.status_code == 200:
                    resp = post.json()
                    print("[✓] ICMP Flow dikirim ke model.")
                    if isinstance(resp, list) and len(resp) > 0:
                        flow_result = resp[-1]
                        result = flow_result.get("result", 1)
                        score = flow_result.get("score", 0.0)
                        result_print = "Benign" if result == 1 else "Malicious"
