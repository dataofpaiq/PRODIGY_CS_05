...
class Flow:
    ...
    def get_data(self) -> dict:
        ...
        # # Handle khusus untuk protokol UDP
        # if self.protocol == 17:  # UDP
        #     data["udp_payload_len"] = sum(len(p[0]['UDP'].payload) for p in self.packets if p[0].haslayer('UDP'))
        #     data["udp_packet_count"] = len([p for p in self.packets if p[0].haslayer('UDP')])

        # # Handle khusus untuk protokol ICMP
        # elif self.protocol == 1:  # ICMP
        #     data.update({
        #         "icmp_type": getattr(self, 'icmp_type', 0),
        #         "icmp_code": getattr(self, 'icmp_code', 0),
        #         "src_port": 0,
        #         "dst_port": 0,
        #     })
    ...
    def add_packet(self, packet: Any, direction: Enum) -> None:
        ...
        # # Periksa layer dan update atribut sesuai protokol
        # if not packet.haslayer('TCP'):
        #     payload_size = len(PacketCount.get_payload(packet))

        #     # Update port untuk UDP
        #     if packet.haslayer('UDP'):
        #         udp_layer = packet.getlayer('UDP')
        #         if self.dest_port == 0:
        #             if direction == PacketDirection.FORWARD:
        #                 self.src_port = udp_layer.sport
        #                 self.dest_port = udp_layer.dport
        #             else:
        #                 self.src_port = udp_layer.dport
        #                 self.dest_port = udp_layer.sport

        #     # Update type & code untuk ICMP
        #     if packet.haslayer('ICMP'):
        #         icmp_layer = packet.getlayer('ICMP')
        #         self.icmp_type = getattr(icmp_layer, 'type', -1)
        #         self.icmp_code = getattr(icmp_layer, 'code', -1)

        # Update flow dan subflow
        self.update_flow_bulk(packet, direction)
        self.update_subflow(packet)

        # Hitung interarrival time
